...
RUN echo "debconf debconf/frontend select Noninteractive" | debconf-set-selections && \
    apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv 8B48AD6246925553 && \
	...
	apt-get upgrade -y && \
    apt-get install -y openjdk-8-jdk binutils jsvc curl cron procps net-tools vim mongodb-org && \
	echo curl -L ${UNIFI_DEB_URL} -o /tmp/unifi.deb && \
    export RUNLEVEL=1 && \
...
# 	echo "deb http://http.debian.net/debian jessie-backports main" > /etc/apt/sources.list.d/100-ubnt.list && \
